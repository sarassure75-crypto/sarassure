...
const TrainerDashboardPage = () => {
  ...
  const fetchLearnerData = useCallback(
    async (learnerId) => {
      if (!learnerId) return;
      ...
      try {
        ...
        toast({
          title: 'Erreur',
          description: "Impossible de charger les données de visibilité de l'apprenant.",
          variant: 'destructive',
        });
      } finally {
  ...
  if (
    !currentUser ||
    (currentUser.role !== USER_ROLES.TRAINER && currentUser.role !== USER_ROLES.ADMIN)
  ) {
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>
          Suivez les progrès et gérez la visibilité des exercices pour chaque apprenant.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="mb-4">
          <label htmlFor="learner-select" className="text-sm font-medium">
              ...
              {learners.length > 0 ? (
                learners.map((learner) => (
                  <SelectItem key={learner.id} value={learner.id}>
                    {learner.first_name || `Apprenant ${learner.learner_code}`}
                  </SelectItem>
                ))
              ) : (
                          ...
                          return (
                            <TableRow
                              ...
                              className={`border-l-4 ${
                                isVisible ? 'border-green-200' : 'border-red-200'
                              } hover:bg-slate-50`}
                            >
                              <TableCell className="pl-6 text-sm">
                                {exIndex === 0 && (
                                  <div className="font-medium pb-1">{task.title}</div>
                                )}
                                ...
                                <div className="flex items-center justify-center space-x-2">
                                  <Switch
                                    ...
                                    onCheckedChange={(checked) =>
                                      handleVisibilityChange(task.id, version.id, checked)
                                    }
