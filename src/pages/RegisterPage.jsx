    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            if (role === USER_ROLES.LEARNER) {
                ...
                if (!profile) {
                     toast({ title: "Inscription presque terminée !", description: "Veuillez vérifier vos emails pour confirmer votre compte. Votre code vous sera accessible après connexion.", duration: 7000 });
                     navigate('/login');
                } else {
            ...
            >
                <Card className="w-full max-w-md shadow-2xl border-2 border-green-400">
                    <CardHeader className="text-center bg-green-500 text-white p-6 rounded-t-lg">
                        ...
                        <CardDescription className="text-green-50">Notez bien vos informations de connexion.</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6 p-6 text-center">
                        ...
                        <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                            <Label className="text-sm text-green-600">Votre Code Secret (pour la connexion simplifiée) :</Label>
                            <p className="text-4xl font-bold tracking-wider text-green-800">{newLearner.learner_code || 'En attente...'}</p>
                        </div>
                        <div className="flex items-start p-3 bg-yellow-50 border border-yellow-300 text-yellow-700 rounded-md text-sm">
                            ...
                            <p className="text-left">Un email de confirmation a été envoyé. Vous devez valider votre email avant de pouvoir vous connecter.</p>
                        </div>
                    </CardContent>
                    <CardFooter>
                        <Button onClick={() => navigate('/login')} className="w-full h-12 text-lg bg-green-600 hover:bg-green-700">
                ...
                <form onSubmit={handleRegister}>
                    <CardContent className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <Input id="firstName" value={firstName} onChange={(e) => setFirstName(e.target.value)} required disabled={loading} placeholder="Votre prénom"/>
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="email">Email</Label>
                            <Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required disabled={loading} placeholder="votre.email@exemple.com"/>
                        </div>
                        <div className="space-y-2">
                    ...
                    <CardFooter className="flex flex-col">
                        <Button type="submit" className="w-full" disabled={loading}>
                            {loading ? "Création en cours..." : <><UserPlus className="mr-2 h-4 w-4" /> S'inscrire</>}
                        </Button>
                         <p className="mt-4 text-xs text-center text-muted-foreground">
                            Déjà un compte ?{" "}
                            <Link to="/login" className="underline underline-offset-2 text-primary hover:text-primary/80">
                                Se connecter
                            </Link>
                        </p>
                        <p className="mt-2 text-xs text-center text-muted-foreground">
                            Apprenant sans email ? Utilisez le{" "}
                            <Link to="/learner-login" className="underline underline-offset-2 text-primary hover:text-primary/80">
