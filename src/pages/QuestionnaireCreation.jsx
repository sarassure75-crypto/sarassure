...
const QuestionnaireCreation = () => {
  ...
  const loadCategories = async () => {
    try {
      const { data, error } = await supabase.from('tasks').select('category').neq('category', null);
  ...
  const validateForm = () => {
    ...
    questions.forEach((q, idx) => {
      ...
      if (choicesWithContent.length < 2) {
        errors.push(
          `Question ${idx + 1}: au moins deux réponses (texte, image ou icône) sont requises`
        );
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const questionsData = questions.map((q, idx) => {
        ...
        const correctAnswers = allChoices.filter((c) => c.isCorrect).map((c) => c.id);

        console.log(
          `✅ Correct answers for Q${idx + 1}:`,
          correctAnswers.length,
          'IDs:',
          correctAnswers.map((id) => id.substring(0, 5))
        );
        ...
        console.log(
          `📊 Q${idx + 1} has ${choicesWithIsCorrect.length} correct choices in final JSON`
        );
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      value={question.text}
                      onChange={(e) =>
                        handleUpdateQuestionText(question.id, 'text', e.target.value)
                      }
                  ...
                  <div>
                    ...
                    <textarea
                      value={question.hint || ''}
                      onChange={(e) =>
                        handleUpdateQuestionText(question.id, 'hint', e.target.value)
                      }
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-900">
                      Chaque réponse peut contenir un texte, une image, une icône ou une combinaison
                      des trois. Marquez au moins une réponse correcte.
                    </div>
                            ...
                            <div className="space-y-3">
                              ...
                              <div>
                                ...
                                <IconSelector
                                  ...
                                  onSelect={(icon) =>
                                    handleUpdateChoiceText(question.id, choice.id, 'icon', icon)
                                  }
                                  onRemove={() =>
                                    handleUpdateChoiceText(question.id, choice.id, 'icon', null)
                                  }
                                  libraries={[
                                    'fa6',
                                    'bs',
