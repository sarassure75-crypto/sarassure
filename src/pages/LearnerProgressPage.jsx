...
const calculatePeriodStats = (progressItems, confidenceMap, startDate, endDate) => {
  ...
  const avgConfidenceBefore =
    confidencesInPeriod.length > 0
      ? (
          confidencesInPeriod.reduce((sum, c) => sum + (c.confidence_before || 0), 0) /
          confidencesInPeriod.length
        ).toFixed(1)
      : 0;
  const avgConfidenceAfter =
    confidencesInPeriod.length > 0
      ? (
          confidencesInPeriod.reduce((sum, c) => sum + (c.confidence_after || 0), 0) /
          confidencesInPeriod.length
        ).toFixed(1)
      : 0;
...
const LearnerProgressPage = () => {
    ...
    try {
      ...
      if (!progressError) {
        toast({
          ...
          description: 'Vos données de suivi sont à jour.',
        });
      }
    } catch (error) {
  ...
  const getTrainerDisplayName = () => {
    ...
    const parts = [
      ...
      trainerProfile.pseudo ? `(${trainerProfile.pseudo})` : '',
    ].filter(Boolean);
    return parts.join(' ');
  };
  ...
  const handleConfidenceAfterChange = async (versionId, newValue) => {
    try {
      ...
      toast({
        title: 'Erreur',
        description: 'Impossible de mettre à jour la confiance.',
        variant: 'destructive',
      });
    }
  };
      ...
      <Card className="mb-6 shadow-lg">
        <CardHeader>
          <CardTitle className="text-3xl flex items-center">
            ...
            Mon Suivi d'Apprentissage
          </CardTitle>
          <CardDescription>
            Bonjour {currentUser.first_name} ! Suivez vos progrès et gérez votre liaison avec un
            formateur.
          </CardDescription>
        </CardHeader>
      </Card>
        ...
        <CardContent>
          {isLoadingTrainer ? (
              ...
              <AlertDialog>
                ...
                <AlertDialogContent className="max-h-[90vh] overflow-y-auto">
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      Cette action est irréversible. Vous devrez contacter votre formateur pour être
                      lié à nouveau.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
            ...
            <p className="text-muted-foreground">
              Vous n'êtes actuellement lié à aucun formateur. Partagez votre code apprenant (
              <strong className="text-primary">{currentUser.learner_code}</strong>) avec votre
              formateur pour qu'il puisse vous lier.
            </p>
          )}
        </CardContent>
      </Card>
                ...
                <CardContent className="space-y-2">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-bold text-blue-900">
                      {parseFloat(stats.avgAttempts).toFixed(1)}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-blue-700">Progression vitesse</span>
                    <span className="font-bold text-green-600">
                      +{parseFloat(stats.speedProgress).toFixed(1)}%
                    </span>
                  </div>
                </CardContent>
              </Card>
        ...
        <CardContent>
          {progressError && (
            <div className="text-red-600 mb-2">
              Erreur : Impossible de charger la progression. Les tâches sont affichées sans suivi.
            </div>
          )}
              ...
              <TableBody>
                {isLoading && progress === null ? (
                  ...
                  progress.map((p, idx) => {
                    ...
                    const isValidFirstTime = isValidTime(
                      p.first_time_seconds,
                      progress
                        .flatMap((item) => [item.first_time_seconds, item.best_time_seconds])
                        .filter(Boolean)
                    );
                    const isValidBestTime = isValidTime(
                      p.best_time_seconds,
                      progress
                        .flatMap((item) => [item.first_time_seconds, item.best_time_seconds])
                        .filter(Boolean)
                    ...
                    const speedProgress =
                      ...
                      p.first_time_seconds &&
                      p.best_time_seconds
                        ? ((p.first_time_seconds - p.best_time_seconds) / p.first_time_seconds) *
                          100
                        : 0;
                        ...
                        <TableCell className="text-center">
                          <span
                            className={`font-semibold ${
                              speedProgress > 0 ? 'text-green-600' : 'text-gray-500'
                            }`}
                          >
                            {speedProgress > 0 ? '+' : ''}
                            {speedProgress.toFixed(1)}%
                          </span>
                        </TableCell>
                        <TableCell className="text-center">
                          <ConfidenceSelector
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="text-center text-muted-foreground">
                      Aucune tâche disponible ou progression.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
                      ...
                      <TableCell className="text-center">
                        <span
                          className={`font-semibold px-3 py-1 rounded-full ${
                            qcm.firstAttemptScore >= 80
                              ? 'bg-green-100 text-green-700'
                              : qcm.firstAttemptScore >= 50
