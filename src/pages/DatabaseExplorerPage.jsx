    ...
    const DatabaseExplorerPage = () => {
        ...
        return (
            <div className="container mx-auto p-4">
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center text-2xl"><Server className="mr-3 text-primary"/>Explorateur de Débogage</CardTitle>
                        <CardDescription>Vue en temps réel des données brutes de votre base Supabase.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        {error ? (
                             <Card className="bg-destructive/10 border-destructive">
                                <CardHeader><CardTitle className="text-destructive flex items-center"><AlertCircle className="mr-2"/>Erreur de chargement</CardTitle></CardHeader>
                            ...
                            <Tabs defaultValue="tasks" className="w-full">
                                <TabsList className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 w-full">
                                    {Object.keys(data).map(tableName => (
                                        <TabsTrigger key={tableName} value={tableName} className="capitalize">{tableName.replace('_', ' ')}</TabsTrigger>
                                    ))}
                                </TabsList>
                                    ...
                                    <TabsContent key={tableName} value={tableName}>
                                        <Card className="mt-4">
                                            <CardHeader className="flex flex-row justify-between items-center">
                                                <div>
                                                    <CardTitle className="capitalize flex items-center"><FileCode2 className="mr-2"/>{tableName.replace('_', ' ')}</CardTitle>
                                                    <CardDescription>Contient {tableData.length} enregistrement(s).</CardDescription>
                                                </div>
                                                <Button variant="outline" size="sm" onClick={() => navigator.clipboard.writeText(JSON.stringify(tableData, null, 2))}>Copier JSON</Button>
                                            </CardHeader>
                                            <CardContent>
