...
const AreaEditor = ({ area, imageDimensions, onMouseDown, onResizeMouseDown, areaType }) => {
  ...
  const style = {
    ...
    backgroundColor: area.color
      ? area.color.replace('0.5', opacity.toString())
  ...
  return (
    <div className="absolute border-2" style={style} onMouseDown={onMouseDown}>
      {handles.map((handleName) => {
        ...
        if (
          ...
          (handleName.includes('top') && handleName.includes('right')) ||
          (handleName.includes('bottom') && handleName.includes('left'))
        ) {
