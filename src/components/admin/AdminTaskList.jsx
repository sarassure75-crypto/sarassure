...
const TaskItem = ({ task, onSelectTask, onDeleteTask, onDuplicateTask, imagesMap }) => {
  const statusInfo = creationStatuses.find((s) => s.id === task.creation_status) || {
    label: 'Inconnu',
    color: 'bg-gray-400',
  };
  const IconComponent = getIconComponent(task.icon_name);
  const PictogramInfo = task.pictogram_app_image_id
    ? imagesMap.get(task.pictogram_app_image_id)
    : null;
      ...
      >
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            {isQuestionnaire && <Badge className="bg-blue-600 text-white">QCM</Badge>}
          </div>
...
const AdminTaskList = ({
  ...
  const otherCats = Object.keys(grouped).filter(
    (k) => k !== 'Sans catégorie' && !categoryOrder.includes(k)
  );
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>
            Créez, modifiez et organisez les tâches de l'application.
          </CardDescription>
        </div>
        ...
        {tasks && tasks.length > 0 ? (
          <div className="space-y-8">
            ...
            {(() => {
              ...
              const categoryOrder = ['all', ...categories.map((c) => c.name)];
              const otherCats = Object.keys(groupedQCM).filter(
                (k) => k !== 'Sans catégorie' && !categoryOrder.includes(k)
              );
              const orderedQCMKeys = [
                ...categories.map((c) => c.name),
                ...otherCats,
                'Sans catégorie',
              ].filter((v, i, a) => a.indexOf(v) === i && groupedQCM[v]);

              const qcmKeysToRender =
            ...
            {(() => {
              ...
              return (
                <div>
                  <h3 className="text-lg font-bold mb-4 flex items-center gap-2">Exercices</h3>
                  <div className="space-y-6">
                    {keysToRender.map((catName) => {
                      ...
                      return (
                        <div key={catName}>
                          {catName !== 'all' && (
                            <h4 className="text-sm font-semibold mb-2">{catName}</h4>
                          )}
                          <div className="space-y-4">
                            {items.map((task) => (
                              <TaskItem
          ...
          <div className="text-center py-12">
            ...
            <Button onClick={onAddNewTask} className="mt-4">
