...
const AdminFaqForm = ({ faqItem: initialFaqItem, onSave, onCancel, images }) => {
  const [faqItem, setFaqItem] = useState(initialFaqItem || { question: '', category: 'Technique', keywords: [], answer_steps: [{ instruction: '', image_id: null, pictogram_id: null }] });
  const [keywordsInput, setKeywordsInput] = useState(initialFaqItem?.keywords?.join(', ') || '');
  const { toast } = useToast();

  useEffect(() => {
    const initialData = initialFaqItem || { question: '', category: 'Technique', keywords: [], answer_steps: [{ instruction: '', image_id: null, pictogram_id: null }] };
  ...
  const handleKeywordsChange = (e) => {
    ...
    setFaqItem(prev => ({ ...prev, keywords: e.target.value.split(',').map(k => k.trim()).filter(Boolean) }));
  };
  ...
  const addStep = () => {
    setFaqItem(prev => ({ ...prev, answer_steps: [...prev.answer_steps, { instruction: '', image_id: null, pictogram_id: null }] }));
  };

  const removeStep = (index) => {
    setFaqItem(prev => ({ ...prev, answer_steps: prev.answer_steps.filter((_, i) => i !== index) }));
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    if (!faqItem.question.trim() || faqItem.answer_steps.some(step => !step.instruction.trim())) {
      toast({ title: "Champs requis", description: "La question et l'instruction de chaque étape sont requises.", variant: "destructive"});
      return;
    }
      ...
      <form onSubmit={handleSubmit}>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <Input id="faq-question" name="question" value={faqItem.question} onChange={handleChange} required />
          </div>
          <div>
            <Label htmlFor="faq-category">Catégorie</Label>
            <Select name="category" value={faqItem.category} onValueChange={(value) => setFaqItem(prev => ({ ...prev, category: value }))}>
              <SelectTrigger id="faq-category"><SelectValue placeholder="Sélectionner une catégorie" /></SelectTrigger>
          ...
          <div>
            ...
            {faqItem.answer_steps.map((step, index) => (
              <AdminFaqStepForm key={index} step={step} index={index} onStepChange={handleStepChange} onRemoveStep={removeStep} images={images} />
            ))}
