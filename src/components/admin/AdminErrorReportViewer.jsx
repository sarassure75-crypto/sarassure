...
const AdminErrorReportViewer = () => {
  ...
  const loadReports = useCallback(
    async (forceRefresh = false) => {
      ...
      try {
        ...
        toast({
          title: 'Erreur',
          description: "Impossible de charger les rapports d'erreurs.",
          variant: 'destructive',
        });
      } finally {
        setIsLoading(false);
  ...
  const handleDelete = async (reportId) => {
    try {
      ...
      toast({
        title: 'Erreur',
        description: 'La suppression du rapport a échoué.',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleMarkAsSent = async (reportId) => {
    try {
      ...
      toast({
        ...
        description: 'Le rapport a été marqué comme traité.',
            ...
            <TableBody>
              {isLoading ? (
                ...
                sortedReports.map((report) => (
                  <TableRow
                    key={report.id}
                    className={!report.is_sent ? 'bg-yellow-50 dark:bg-yellow-900/20' : ''}
                  >
                    <TableCell>
                      <Badge
                        variant={report.is_sent ? 'default' : 'secondary'}
                        className={report.is_sent ? 'bg-green-600' : ''}
                      >
                    ...
                    <TableCell className="text-sm text-muted-foreground">
                      ...
                      <div className="text-xs">
                        {report.version_name
                          ? `${report.version_name} (${report.version_android || 'N/A'})`
                          : ''}
                      </div>
                    </TableCell>
                    <TableCell>
                      {formatDistanceToNow(new Date(report.report_date), {
                        addSuffix: true,
                        locale: fr,
                      })}
                    </TableCell>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[600px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Rapport de {selectedReport?.user_first_name || 'Anonyme'} du{' '}
                {selectedReport ? new Date(selectedReport.report_date).toLocaleString('fr-FR') : ''}
              </DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4 max-h-[70vh] overflow-y-auto px-2">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <p className="col-span-3">
                  {selectedReport?.version_name
                    ? `${selectedReport.version_name} (Android ${selectedReport.version_android})`
                    : 'N/A'}
                </p>
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
            ...
            <DialogFooter>
              {selectedReport?.task_id && selectedReport?.version_id && (
                <Button asChild variant="outline">
                  <Link
                    to={`/admin/preview/tache/${selectedReport.task_id}/version/${
                      selectedReport.version_id
                    }?stepIndex=${(selectedReport.step_index || 1) - 1}`}
                  >
                    Voir l'étape
                  </Link>
                </Button>
