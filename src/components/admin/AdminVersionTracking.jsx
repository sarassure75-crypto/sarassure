...
const AdminVersionTracking = () => {
  ...
  useEffect(() => {
    if (tasksData) {
      const processedVersions = tasksData.flatMap(task => 
        (task.versions || []).map(version => ({ ...version, taskTitle: task.title, taskId: task.id }))
      );
  ...
  const handleStatusChange = async (versionId, newStatus) => {
    ...
    try {
      ...
      toast({ title: "Statut mis à jour", description: `Le statut de la version a été changé en "${creationStatuses.find(s => s.id === newStatus)?.label}".` });
    } catch (error) {
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Visualisez et gérez l'état d'avancement de la création des versions.</CardDescription>
        </CardHeader>
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Suivi de Création des Versions</CardTitle>
        <CardDescription>Visualisez et gérez directement l'état d'avancement de la création des versions.</CardDescription>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
              ...
              allVersions.map(version => {
                ...
                return (
                  <React.Fragment key={`${version.taskId}-${version.id}`}>
                    <TableRow>
                      <TableCell>
                        <div className="font-medium flex items-center cursor-pointer" onClick={() => toggleExpandVersion(version.id)}>
                            {expandedVersions[version.id] ? <ChevronUp className="h-4 w-4 mr-2"/> : <ChevronDown className="h-4 w-4 mr-2"/>}
                            <div>
                                <div>{version.taskTitle}</div>
                                <div className="text-sm text-muted-foreground">{version.name} ({version.version || 'N/A'})</div>
                            </div>
                        </div>
                      </TableCell>
                      <TableCell>
                          ...
                          <SelectContent>
                            {creationStatuses.map(status => (
                              <SelectItem key={status.id} value={status.id}>
                                <Badge className={`${statusInfo.color} text-white`}>{status.label}</Badge>
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                        ...
                        <TableRow className="bg-muted/30">
                            <TableCell colSpan={2} className="py-2 px-8">
                                ...
                                {version.steps && version.steps.length > 0 ? (
                                    ...
                                    <p className="text-xs text-muted-foreground italic">Aucune étape définie pour cette version.</p>
                                )}
                            </TableCell>
                        </TableRow>
                    )}
