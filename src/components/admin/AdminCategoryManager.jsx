...
const AdminCategoryManager = () => {
  ...
  const handleSaveEdit = async () => {
    ...
    if (
      ...
      toast({
        ...
        description: 'Ce nom de catégorie existe déjà.',
        variant: 'destructive',
      });
      return;
    }
    ...
    toast({
      ...
      description: `"${editingCategory.name}" est devenu "${newCategoryName.trim()}".`,
    });
  ...
  const handleDelete = async (categoryToDelete) => {
    if (categoryToDelete.name === 'Général') {
      toast({
        ...
        description: "La catégorie 'Général' ne peut pas être supprimée.",
        variant: 'destructive',
      });
      return;
    }
  ...
  const handleAddCategory = async () => {
    ...
    toast({
      ...
      description: `La catégorie "${trimmedName}" a été ajoutée.`,
    });
    setNewCategoryInput('');
  ...
  const handleAddSubcategory = async () => {
    ...
    if (!trimmedName) {
      toast({
        title: 'Nom invalide',
        description: 'Le nom de la sous-catégorie ne peut pas être vide.',
        variant: 'destructive',
      });
      return;
    }
    if (!selectedParentCategory) {
      toast({
        title: 'Catégorie parente manquante',
    ...
    toast({
      ...
      description: `La sous-catégorie "${trimmedName}" a été ajoutée.`,
    });
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Modifiez, ajoutez ou supprimez des catégories et sous-catégories. La suppression d'une
          catégorie nécessite une réaffectation manuelle des tâches.
        </CardDescription>
      </CardHeader>
          ...
          <Dialog open={isAddSubcategoryDialogOpen} onOpenChange={setIsAddSubcategoryDialogOpen}>
            <DialogTrigger asChild>
              <Button size="sm" className="flex-1" variant="outline">
                <PlusCircle className="mr-2 h-4 w-4" /> Ajouter une Sous-Catégorie
              </Button>
            </DialogTrigger>
            <DialogContent className="max-h-[90vh] overflow-y-auto">
              <form
              ...
              >
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Sélectionnez une catégorie parente et entrez le nom de la sous-catégorie.
                  </DialogDescription>
                </DialogHeader>
                <div className="grid gap-4 py-4">
                  <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <div className="grid grid-cols-4 items-center gap-4">
                    <Label htmlFor="new-subcategory-name" className="text-right">
                      Nom
                    </Label>
                    <Input
                      ...
                      >
                        <ChevronRight
                          className={`h-4 w-4 transition-transform ${
                            expandedCategories[parentCategory.id] ? 'rotate-90' : ''
                          }`}
                        />
                      </button>
                    )}
                  ...
                  parentCategory.subcategories.length > 0 && (
                    <ul className="ml-8 space-y-2 mt-2">
                      {parentCategory.subcategories.map((subcategory) => (
                        <li
                          key={subcategory.id}
                          className="flex items-center justify-between p-3 border border-l-4 border-l-blue-400 rounded-md bg-blue-50 hover:bg-blue-100 transition-colors"
                        >
