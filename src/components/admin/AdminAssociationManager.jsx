...
import { getUnassignedLearners, assignTrainerToLearner, unassignTrainerFromLearner, getAllUsersByRole, getLearnersByTrainer } from '@/data/users';
...
const AdminAssociationManager = () => {
    ...
    const fetchAssignedLearners = useCallback(async (trainerId) => {
        if (!trainerId) return;
        setIsLoading(true);
        try {
            ...
            toast({ title: 'Erreur', description: `Impossible de charger les apprenants pour ce formateur.`, variant: 'destructive' });
        } finally {
            setIsLoading(false);
        }
    }, [toast]);
    ...
    const handleAssign = async () => {
        if (!selectedTrainerId || !selectedLearnerToAssign) {
            toast({ title: 'Sélection requise', description: 'Veuillez sélectionner un formateur et un apprenant.', variant: 'destructive' });
            return;
        }
        setIsAssigning(true);
        try {
            await assignTrainerToLearner(selectedLearnerToAssign, selectedTrainerId);
            toast({ title: 'Association réussie !', description: 'L\'apprenant a été assigné au formateur.' });
    ...
    const handleUnassign = async (learnerId) => {
        try {
            ...
            toast({ title: 'Dissociation réussie !', description: 'L\'apprenant n\'est plus assigné à ce formateur.' });
             // Refresh data
            const unassigned = await getUnassignedLearners();
            setUnassignedLearners(unassigned);
            await fetchAssignedLearners(selectedTrainerId);
    ...
    return (
        <div className="grid md:grid-cols-2 gap-6">
            <Card>
                <CardHeader>
                    <CardTitle className="flex items-center"><Link className="mr-2 h-5 w-5"/>Associer un Apprenant</CardTitle>
                    <CardDescription>Sélectionnez un formateur, puis un apprenant à lui assigner.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    <div>
                        ...
                        <Select onValueChange={setSelectedTrainerId} value={selectedTrainerId || ''}>
                            ...
                            <SelectContent>
                                {trainers.map(t => <SelectItem key={t.id} value={t.id}>{t.first_name} ({t.pseudo})</SelectItem>)}
                            </SelectContent>
                        </Select>
                    </div>
                    <div>
                        <label className="text-sm font-medium">Apprenant non assigné</label>
                        <Select onValueChange={setSelectedLearnerToAssign} value={selectedLearnerToAssign || ''}>
                            <SelectTrigger>
                                <SelectValue placeholder="Sélectionnez un apprenant" />
                            </SelectTrigger>
                            <SelectContent>
                                {unassignedLearners.length > 0 ? (
                                    unassignedLearners.map(l => <SelectItem key={l.id} value={l.id}>{l.first_name} ({l.learner_code})</SelectItem>)
                                ) : (
                    ...
                    <Button onClick={handleAssign} disabled={!selectedTrainerId || !selectedLearnerToAssign || isAssigning}>
                ...
                <CardHeader>
                    <CardTitle className="flex items-center justify-between">
                        <div className="flex items-center">
                            <Users2 className="mr-2 h-5 w-5"/>Apprenants Assignés
                        </div>
                        <Button variant="ghost" size="icon" onClick={() => { if(selectedTrainerId) fetchAssignedLearners(selectedTrainerId) }} disabled={!selectedTrainerId || isLoading}>
                            <RefreshCw className={`h-4 w-4 ${isLoading ? 'animate-spin' : ''}`} />
                        </Button>
                    </CardTitle>
                    <CardDescription>
                        {selectedTrainerId ? `Liste des apprenants assignés à ${trainers.find(t => t.id === selectedTrainerId)?.first_name || ''}` : 'Sélectionnez un formateur pour voir ses apprenants.'}
                    </CardDescription>
                </CardHeader>
                <CardContent>
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {isLoading ? (
                                    ...
                                    assignedLearners.map(learner => (
                                        <TableRow key={learner.id}>
                                            ...
                                            <TableCell className="text-right">
                                                <Button variant="destructive" size="sm" onClick={() => handleUnassign(learner.id)}>
                                    ...
                                    <TableRow><TableCell colSpan={2} className="text-center text-muted-foreground">
                                        {selectedTrainerId ? 'Aucun apprenant assigné.' : 'Veuillez sélectionner un formateur.'}
                                    </TableCell></TableRow>
                                )}
                            </TableBody>
                        </Table>
