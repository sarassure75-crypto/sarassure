...
import { getAvailableLanguages, translateInstruction, getGlossaryTranslations, createGlossaryTranslation, updateGlossaryTranslation, deleteGlossaryTranslation } from '@/data/translation';
...
export function TranslatableText({ 
  ...
  return (
    <div className={cn('relative', className)}>
      ...
      <p className={cn('text-gray-800', isTranslating && 'opacity-50')}>
        {translatedText}
      </p>
    </div>
  );
...
export function AdminTranslationManager() {
  ...
  const filteredGlossaryTerms = glossarySearchText.trim() === '' 
    ? [] 
    : allGlossaryTerms.filter(term => 
  ...
  const handleSaveEdit = async (translationId) => {
    try {
      ...
      setGlossaryTerms(glossaryTerms.map(t => 
        t.id === translationId ? { ...t, ...editingData } : t
      ));
      setEditingId(null);
      toast({
        title: "Succès",
  ...
  const handleAddTranslation = async () => {
    if (!newTranslation.glossary_id || !newTranslation.translated_term || !newTranslation.translated_definition) {
      toast({
        title: "Erreur",
        description: "Veuillez remplir tous les champs obligatoires (y compris sélectionner un terme du lexique)",
        variant: "destructive"
      });
      return;
    }
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setNewTranslation({...newTranslation, translated_term: e.target.value})}
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) => setNewTranslation({...newTranslation, translated_definition: e.target.value})}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setNewTranslation({...newTranslation, translated_example: e.target.value})}
        ...
        {glossaryTerms.length === 0 ? (
          ...
          <div className="grid gap-3">
            {glossaryTerms.map(term => (
              <Card key={term.id} className={editingId === term.id ? 'border-blue-500 bg-blue-50' : ''}>
                <CardContent className="pt-6">
                  {editingId === term.id ? (
                    <div className="space-y-3">
                      <input
                        ...
                        onChange={(e) => setEditingData({...editingData, translated_term: e.target.value})}
                      ...
                      <textarea
                        ...
                        onChange={(e) => setEditingData({...editingData, translated_definition: e.target.value})}
                        rows={2}
                      ...
                      <input
                        ...
                        onChange={(e) => setEditingData({...editingData, translated_example: e.target.value})}
                        placeholder="Exemple..."
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-gray-500">Terme original: <strong>{term.glossary.term}</strong></p>
