...
const InformationPanel = ({ isVisible, onClose, actionType, startArea }) => {
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-start gap-2 flex-1">
            ...
            <div className="text-xs text-blue-700">
              <p className="font-semibold">Action: {getActionLabel()}</p>
            </div>
          </div>
          <Button onClick={onClose} variant="ghost" size="sm" className="h-5 w-5 p-0 flex-shrink-0">
            <X className="h-3 w-3" />
          </Button>
        </div>
      </motion.div>
