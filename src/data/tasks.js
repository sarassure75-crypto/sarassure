...
export const fetchTasks = async (forceRefresh = false) => {
  try {
    ...
    const { data: tasks, error: tasksError } = await query.order('created_at', {
      ascending: false,
    });
...
export const updateTask = async (taskId, updates) => {
  const { data, error } = await supabase
    .from('tasks')
    .upsert({ ...updates, id: taskId }, { onConflict: 'id' })
    .select();
  ...
  try {
    ...
    if (!versionData.id) {
      ...
      if (error) {
        ...
        if (fallbackError) throw fallbackError;
        if (!fallbackData || fallbackData.length === 0)
          throw new Error("L'enregistrement de la version a échoué.");
        return fallbackData[0];
      }
