...
export function useAdminRevenue(adminId) {
  ...
  const loadAdminData = useCallback(async () => {
    try {
      ...
      logger.log(
        '🔍 DEBUGGING: Checking the problematic version 8f637b93-22a5-474d-8805-fb06f46356b3'
      );
      ...
      const { data: allVersionsData, error: versionsError } = await supabase.from('versions')
        .select(`
          id,
          creation_status,
          task_id,
      ...
      logger.log(
        '🔍 RAW versions data (first 5):',
        allVersionsData?.slice(0, 5).map((v) => ({
          id: v.id,
      ...
      const allValidatedVersions =
        allVersionsData?.filter((v) => v.creation_status === 'validated') || [];
      ...
      const contributorValidatedVersions =
        allVersionsData?.filter((v) => {
      ...
      logger.log(
        '🎯 FINAL Contributor validated versions count:',
        contributorValidatedVersionsCount
      );
      logger.log(
        '🎯 Contributor versions details:',
        contributorValidatedVersions.map((v) => ({
          id: v.id,
      ...
      const contributorValidatedImages =
        allImagesData?.filter(
          (img) => img.uploaded_by !== adminId && img.moderation_status === 'approved' // Images use 'approved', not 'validated'
        ) || [];
